...
const MyPathDataContext = React.createContext<MyPathDataContextType>({
...
export const useMyPathDataContext = (): MyPathDataContextType =>
  useContext(MyPathDataContext);

export const MyPathDataProvider: React.FC<MyPathDataProviderProperties> = ({
  children,
}) => {
  ...
  useEffect(() => {
    if (performingUndoRedo.current) {
      ...
      !loadingNewFile.current &&
      (JSON.stringify(myPathData.pathData) !==
        JSON.stringify(previousMyPathData.current.pathData) ||
        JSON.stringify(myPathData.imageData) !==
          JSON.stringify(previousMyPathData.current.imageData))
    ) {
