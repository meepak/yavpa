...
class Vector {
	...
	roundAngle() {
		...
		const module_ = utils.fixedMod(rotation, this.rotationProduct);
		const sign = (Math.abs(module_) / module_) || 1;

		if (module_ > this.rotationProduct / 2) {
			return this.rotate(-sign * (module_ - this.rotationProduct));
		}

		return this.rotate(-sign * module_);
	}
}
