...
class Polygon {
	...
	set vertices(vertices) {
		...
		return this._vertices = Vertex.map(vertices).filter((currentVertex, i) => {
			const previousIndex = utils.fixedMod(i - 1, vertices.length);
			const previousVertex = vertices[previousIndex];

			return !utils.isSimilar(currentVertex.x, previousVertex.x)
             || !utils.isSimilar(currentVertex.y, previousVertex.y);
		});
	}
	...
	getRatios(vectors = this.getVectors()) {
		const unitVector = vectors.reduce((unitVector, vector) => vector.getLength() < unitVector.getLength() ? vector : unitVector);

		return vectors.map(vector => vector.getLength() / unitVector.getLength());
	}
	...
	getCenter() {
		...
		for (const [currentIndex, currentVertex] of vertices.entries()) {
			const nextIndex = (currentIndex + 1) % vertices.length;
			const nextVertex = vertices[nextIndex];

			const areaDiff = currentVertex.x * nextVertex.y - nextVertex.x * currentVertex.y;
			areaSum += areaDiff;

			x += (currentVertex.x + nextVertex.x) * areaDiff;
			y += (currentVertex.y + nextVertex.y) * areaDiff;
		}

		// If this is a straight line
		if (!areaSum) {
			return vertices.reduce((sumVertex, currentVertex) => ({
				x: sumVertex.x + (currentVertex.x / this.vertices.length),
				y: sumVertex.y + (currentVertex.y / this.vertices.length),
			}), {
				x: 0,
				y: 0,
			});
	...
	getBoundingBox() {
		...
		// Changed from Rectangle to avoid circular dependency
		return new Polygon([
			{x: minX, y: minY},
			{x: maxX, y: minY},
			{x: maxX, y: maxY},
	...
	fitWith(polygon, useMirroring = true) {
		...
		for (let i = 0; i < polygon.vertices.length; i++) {
			...
			let currentRadian = utils.fixedMod(
				Math.PI / 2 - polygonVector.getAngle(selfVector),
				Math.PI / 2,
			);
			currentRadian = Math.min(currentRadian, Math.PI / 2 - currentRadian);

			radians.push(currentRadian);
		...
		if (useMirroring) {
			...
			const asymmetric = this.vertices.some((vertex, index) => !_.isEqual(vertex, mirroredSelf.vertices[index]));
		...
		return _.minBy(candiPolygons, candiPolygon => _.sum(candiPolygon.vertices.map(candiVertex => _.min(polygon.vertices.map(polygonVertex => {
	...
	reduceLOD(threshold) {
		...
		// The cosines that we would **not** like to keep
			isNaN(cosine) || utils.isBetweenThreshold(cosine, Math.PI, threshold),
		)
		// Filter subsequent cosines and treat them as a single angle
			.map((testResult, index) => !testResult && this.vertices[index])
			.filter(Boolean);
		...
		if (vertices.length < 3) {
			return new Polygon(
				[this.vertices[0], this.vertices.at(-1)], this.options,
			);
		}
		...
		for (const vertex of vertices) {
			for (const [candiIndex, candiVertex] of vertices.entries()) {
				if (vertex === candiVertex) {
					continue;
				}
