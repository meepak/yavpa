...
function createShapeit(config = {}) {
	...
	function detectShape(vertices) {
		...
		for (let i = 2; i < vertices.length - 1; i++) {
			...
			for (let index = 0; index < i - 1; index++) {
				const candiVertex = vertices[index];
				const nextCandiVertex = vertices[index + 1];
            ...
            || utils.isBetweenThreshold(angle, Math.PI, thresholds.vectorsReductionAngle)) {
				...
				if (intersectionVertex === Vertex.NaN) {
					continue;
				}

				const subVertices = vertices.slice(index + 1, i).concat(intersectionVertex);
		...
		const matchingPatternIndex = _.maxBy(Object.keys(scores), index => scores[index]);
	...
	function tryCircle(polygon) {
		...
		if (polygon.reduceLOD(thresholds.circleReductionAngle).vertices.length > 3) {
			return;
		}
		...
		if (closureVector.getLength() > thresholds.circleClosureDistance) {
			return;
		}

		const center = polygon.getCenter();

		const radiuses = polygon.vertices.map(vertex => new Vector(center, vertex).getLength());
		...
		if (STDratio > thresholds.radiusesStdRatio) {
			return;
		}
	...
	function handleQuadShape(shape) {
		...
		if (score < thresholds.minSquareScore) {
			return;
		}
	...
	Object.assign(shapeit, {
		...
		modify(module_ = {}) {
			...
			atlas.push(...transformAtlas(module_.atlas));
			Object.assign(output, module_.output);
			Object.assign(thresholds, module_.thresholds);

			// Rotation segment for stored square shape should be updated
			if (module_.output.rectRotationProduct != null) {
				const square = atlas.find(({name}) => name == 'square');
				square.rotationProduct = module_.output.rectRotationProduct;
			}
