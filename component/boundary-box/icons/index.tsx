...
>();
  ...
  const iconBoxWidth = 36 / canvasScale;
  ...
  const start = {
    ...
    y:
      Number.parseFloat(vbbPoints[1]) + Number.parseFloat(canvasPoints[1]),
  };

  if (start.x + iconBoxWidth * 2 > CANVAS_WIDTH * canvasScale) {
    start.x = CANVAS_WIDTH * canvasScale - iconBoxWidth * 2;
  }

  if (start.y + iconBoxHeight > CANVAS_HEIGHT * canvasScale) {
    start.y = CANVAS_HEIGHT * canvasScale - iconBoxHeight;
  }

  const top = start.y - 15*canvasScale < 0 ? 0 : start.y;
