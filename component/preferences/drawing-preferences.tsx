...
import { createPathdata, getDeviceOrientation, getPathFromPoints, getPenOffsetFactor } from "@u/helper";
import { MY_BLACK, Orientation, OrientationType, PointType, SCREEN_WIDTH } from "@u/types";
...
const DrawingPreferences: React.FC<{ disableParentScroll: (val: boolean) => void }> = ({ disableParentScroll }) => {
    ...
    const myCircleAnim = useRef(new Animated.Value(0)).current;
    const colorAnim = useRef(new Animated.Value(0)).current; // 0: yellow, 1: sky light blue
    ...
    pan.onUpdate((event) => {
        ...
        setSamplePathData((prev) => ({ ...prev, path: getPathFromPoints(points.current) }));

        setPenTip({ x: x, y: y } as any);
    ...
    return (
        <View style={{ marginHorizontal: 15, marginVertical: 20 }}>
            ...
            <View style={{ width: SCREEN_WIDTH, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                <MySlider
                    name="X Offset"
                    value={penOffset.x || 0}
            ...
            <ImageBackground source={require('@a/small-canvas.png')} style={{ width: SCREEN_WIDTH - 30, height: 300 }}>
                ...
                <GestureHandlerRootView style={{ width: SCREEN_WIDTH }}>
                    <GestureDetector gesture={pan}>
                        <View
                            style={{ left: 7, width: SCREEN_WIDTH - 45, height: 265, backgroundColor: 'transparent' }}
                        >
                            <Svg
                                width='100%'
                                height={'100%'}
