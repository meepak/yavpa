...
import {
	View, Text, TouchableOpacity, StyleSheet, Alert, TouchableWithoutFeedback,
} from 'react-native';
import {
	CANVAS_WIDTH, MY_BLACK, type PathDataType, type MyPathDataType, CANVAS_HEIGHT,
} from '@u/types';
...
const PathsAsLayersDraw = () => {
			...
			{text: 'Cancel', style: 'cancel'},
			{
				...
				onPress() {
					...
					setMyPathData((previousMyPathData: MyPathDataType) => {
						const newLayers = previousMyPathData.pathData.filter(path => path !== item);
						return {...previousMyPathData, metaData: {...previousMyPathData.metaData, updatedAt: ''}, pathData: newLayers};
					});
				},
			},
		]);
		...
		Alert.alert('Delete Path', 'Are you sure you want to delete all selected paths permanently?', [
			{text: 'Cancel', style: 'cancel'},
			{
				...
				onPress() {
					// Delete indexth element from myPathData.pathData
					setMyPathData((previousMyPathData: MyPathDataType) => {
						const newLayers = previousMyPathData.pathData.filter(path => !path.selected);
						return {...previousMyPathData, metaData: {...previousMyPathData.metaData, updatedAt: ''}, pathData: newLayers};
					});
				},
			},
		...
		Alert.alert('Combine Paths', 'Are you sure you want to combine all selected paths into one?', [
			{text: 'Cancel', style: 'cancel'},
			{
				...
				onPress() {
					// Delete indexth element from myPathData.pathData
					setMyPathData((previousMyPathData: MyPathDataType) => {
						const selectedPaths = [...previousMyPathData.pathData].filter(path => path.selected);
						const unselectedPaths = [...previousMyPathData.pathData].filter(path => !path.selected);
						...
						if (selectedPaths.length > 0) {
							const combinedPath = selectedPaths.reduce((combined, current, index) => {
								...
								const match = /m\s*([-\d.]+)\s*,?\s*([-\d.]+)/i.exec(current.path);
								if (match) {
									const [, x, y] = match;
							...
							// Get new path length
							const length = getPathLength(getPointsFromPath(combinedPath));

							unselectedPaths.push({
								...selectedPaths[0], path: combinedPath, length, time: totalTime,
							});
						}

						return {
							...previousMyPathData, metaData: {...previousMyPathData.metaData, updatedAt: ''}, pathData: unselectedPaths, updatedAt: new Date().toISOString(),
						};
					});
				},
			},
	...
	function reversePaths() {
		setMyPathData((previous: MyPathDataType) => {
			...
			return {...previous, metaData: {...previous.metaData, updatedAt: ''}, pathData: newLayers};
		});
	}
	...
	function deleteImage() {
		Alert.alert('Delete Image', 'Are you sure you want to delete this image permanently?', [
			{text: 'Cancel', style: 'cancel'},
			{
				text: 'Delete',
				onPress() {
					setMyPathData((previousMyPathData: MyPathDataType) => ({...previousMyPathData, metaData: {...previousMyPathData.metaData, updatedAt: ''}, imageData: []}));
				},
			},
		]);
	}
	...
	const handlePathUpdate = (path: PathDataType, property: any, value: any) => {
		setMyPathData((previousMyPathData: MyPathDataType) => {
			...
			const newMeta = {...previousMyPathData.metaData, updatedAt: property === 'selected' ? new Date().toISOString() : ''};
			return {
				...previousMyPathData, metaData: newMeta, pathData: newLayers, updatedAt: new Date().toISOString(),
			};
		});
	};
	...
	const toggleAllSelection = (checked: boolean) => {
		...
		setMyPathData((previousMyPathData: MyPathDataType) => {
			const newLayers = previousMyPathData.pathData.map(path => ({...path, selected: checked}));
			return {...previousMyPathData, metaData: {...previousMyPathData.metaData, updatedAt: ''}, pathData: newLayers};
		});
	};
	...
	const renderItem = ({item, drag, isActive}) => {
		...
		return (
			<ScaleDecorator key={item.guid}>
				<View
					style={{
						...styles.cell, width: 180, padding: 5, backgroundColor: isActive ? 'rgba(0,0,255,0.2)' : 'transparent',
					}}>
							...
							<View style={styles.row}>
								...
								<TouchableOpacity style={{...styles.cell}}>
									<ContextMenu
										...
										height={500}
									>
										<SelectBrushColor value={item.stroke} onValueChanged={(color: any) => {
					...
					onDragEnd={data => {
						if (data.from !== data.to) {
							setMyPathData((previousMyPathData: MyPathDataType) => {
								...
								return {...previousMyPathData, metaData: {...previousMyPathData.metaData, updatedAt: ''}, pathData: newData};
							});
						}
					}}
