...
const Header = ({
  ...
  const handleScreenModeButtonPress = () => {
    ...
    const currentScreenModeIndex = ScreenModes.findIndex(
      (mode) => mode.name === screenMode.name,
    );
    if (
      currentScreenModeIndex >= 0 &&
      ScreenModes[currentScreenModeIndex].name === "Draw"
    ) {
      // Get count of visible paths
      const pathsOnScreen = myPathData.pathData.reduce(
        (accumulator, path) => (path.visible ? accumulator + 1 : accumulator),
        0,
      );
      if (pathsOnScreen === 0) {
