...
import {
	type AnimationParamsType, CANVAS_VIEWBOX_DEFAULT, type MyPathDataType, TransitionType,
} from '@u/types';
...
const SvgAnimate = React.forwardRef((properties: Properties, reference) => {
	...
	const viewBox = properties.myPathData.metaData.viewBox || CANVAS_VIEWBOX_DEFAULT; // Props.myPathData.metaData.viewBox;
	...
	const saveAnimationParameters = (animationValues: AnimationParamsType) => {
		if (JSON.stringify(animationValues) !== JSON.stringify(animationParameters)) {
	...
	const animatedValuesReference = useRef(pathData.map(() => new Animated.Value(0)));
	...
	const getAnimations = () => Animated.sequence([
		...
		...(animationParameters.loop ? [
			...
			...(animatedValuesReference.current).map(animatedValue => Animated.timing(animatedValue, {
				toValue: 0,
				duration: 0,
				useNativeDriver: false,
			})),
	...
	const stopAnimation = () => {
		...
		if (properties.onLoopEnd) {
			animatedValuesReference.current.at(-1).removeAllListeners();
		}
	};
        ...
        || animationParameters.transitionType === TransitionType.Shake
			...
			>
				...
				{pathData.map((path, index) => {
					const strokeDasharray = path.length * (1 + animationParameters.correction);
					const strokeDashoffset = animatedValuesReference.current[index].interpolate({
						inputRange: [0, 1],
