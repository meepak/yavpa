...
export function getMyOnPrimaryColor() {
	const randomIndex = Math.floor(Math.random() * MY_ON_PRIMARY_COLOR_OPTIONS.length);
...
export const precise = (number_: string | number, precision = PRECISION): number =>
...
export const getPointsFromPath = (
	...
	let firstPoint: PointType | undefined = null;
	let previousPoint: PointType | undefined = null;
	const points: PointType[] = [];
	for (const command of commands) {
		const type = command[0].toUpperCase();
		const coords = command.slice(1).split(',').map(Number);
		let point: PointType | undefined = null;
		switch (type) {
			case 'C': {
	...
	const simplifiedPoints = (points.length > 10)
		? simplify(points, simplifyTolerance)
		: points;
...
export const getLastPoint = (path: string) => {
	...
	const x = parameters.at(-2);
	const y = parameters.at(-1);
...
export const getPathLength = (points: PointType[]): number => polygonLength(points.map(point => [point.x, point.y]));

export const getSnappingPoint = (paths: PathDataType[], screenPoint: PointType, canvasScale: number, canvasTranslate: PointType) => {
	...
	for (const path of paths) {
		...
		if (currentPoint) {
			const distanceFromFirst = calculateDistance(
				firstPoint,
				currentPoint,
			);
	...
	} else { // TODO FIND SNAP POINT WITHIN PATH
...
export const getDeviceOrientation = async () => new Promise(resolve => {
