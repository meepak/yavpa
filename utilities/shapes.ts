...
import { getPathFromPoints, precise, calculateDistance } from "./helper";
import { AvailableShapes, type PointType, type ShapeType } from "./types";
...
export const isValidShape = (name: string) =>
  AvailableShapes.includes(name) && name !== AvailableShapes[0];
...
/*
 * Direct SVG path for a circle,  will experiment with later
 * https://stackoverflow.com/questions/5737975/circle-drawing-with-svgs-arc-path
 */
export function circlePathDirect(cx: number, cy: number, r: number) {
  return (
    "M " +
    cx +
    " " +
...
const PolygonPath = (start: PointType, end: PointType, sides: number) => {
  ...
  for (let i = 0; i <= sides; i++) {
    // Loop until sides + 1 to include the last point
...
export const getD3CurveBasis = (
  d3Value: string,
  validShape: boolean,
): d3.CurveFactory | d3.CurveFactoryLineOnly | undefined => {
  switch (d3Value) {
    case "auto": {
