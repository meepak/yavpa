...
export const getFile = async (documentDir: string, guid: string): Promise<MyPathDataType | undefined> => {
	try {
		const appSaveDir = getAppSavePath(documentDir);
...
export const saveImageToCache = async (documentDir: string, filePath: string, height: number, width: number): Promise<any | undefined> => {
	const appSaveDir = getAppSavePath(documentDir);
