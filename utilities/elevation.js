...
const derive = (i, a, b, a2, b2) => ((i - a) * (b2 - a2)) / (b - a) + a2;

const parseShadow = raw => {
	const values = raw.split(' ').map(value => Number(value.replace('px', '')));
	return {
		x: values[0],
		y: values[1],
		blur: values[2],
...
const elevations = Array.from({length: maxElevation + 1})
	.fill(undefined)
	.map((x, index) => generateElevationStyle(index));

elevations.interpolate = (animation, {inputRange, outputRange, ...options}) => {
	//  Let style = {}
	//  let style = {
	//  	backgroundColor:'#f00',
	//  	opacity:"var(--elev)",
	...
	if (I_AM_ANDROID) {
		style = {
			elevation: animation.interpolate({inputRange, outputRange, ...options}),
		};
	} else if (I_AM_IOS) {
		const output = {height: [], shadowOpacity: [], shadowRadius: []};
		for (const depth of outputRange) {
			const s = parseShadow(webDepth.penumbra[depth]);
			const y = s.y === 1 ? 1 : Math.floor(s.y * 0.5);
		...
		style = {
			...
			shadowOffset: {
				width: 0,
				height: animation.interpolate({inputRange, outputRange: output.height, ...options}),
			},
			shadowOpacity: animation.interpolate({inputRange, outputRange: output.shadowOpacity, ...options}),
			shadowRadius: animation.interpolate({inputRange, outputRange: output.shadowRadius, ...options}),
		};
	} else if (Platform.OS === 'web') {
		const output = {
			elevation: [], zi: [], u1: [], u2: [], u3: [], p1: [], p2: [], p3: [], a1: [], a2: [], a3: [],
		};
		for (const depth of outputRange) {
			output.elevation.push(depth);
			output.zi.push(String(depth));
			const u = webDepth.umbra[depth].split(' ');
			const p = webDepth.penumbra[depth].split(' ');
			const a = webDepth.ambient[Math.max(0, depth - 1)].split(' ');
			output.u1.push(u[1]);
			output.u2.push(u[2]);
			output.u3.push(u[3]);
			output.p1.push(p[1]);
		...
		style = {
			elevation: animation.interpolate({inputRange, outputRange: output.elevation, ...options}),
			zIndex: animation.interpolate({inputRange, outputRange: output.zi, ...options}),
			'--u1': animation.interpolate({inputRange, outputRange: output.u1, ...options}),
			'--u2': animation.interpolate({inputRange, outputRange: output.u2, ...options}),
			'--u3': animation.interpolate({inputRange, outputRange: output.u3, ...options}),
			'--p1': animation.interpolate({inputRange, outputRange: output.p1, ...options}),
			'--p2': animation.interpolate({inputRange, outputRange: output.p2, ...options}),
			'--p3': animation.interpolate({inputRange, outputRange: output.p3, ...options}),
			'--a1': animation.interpolate({inputRange, outputRange: output.a1, ...options}),
			'--a2': animation.interpolate({inputRange, outputRange: output.a2, ...options}),
			'--a3': animation.interpolate({inputRange, outputRange: output.a3, ...options}),
